...
* Asynch connection handling by default, but you can force synch by adding `Synchronous true` to a `<Connection>` block. If the url contains `remoting-jmx` (JBoss Remoting) synchronous is automagically enabled.
